...
- (id)init
{
    self = [[[NSBundle mainBundle] loadNibNamed:@"SwLanguagePop"
                                          owner:self
...
-(void)UpdateView
{
    ...
    [title setText:[SwichLanguage getString:@"languageswitch"]];
    [label1 setText:[SwichLanguage getString:@"chinese"]];
    [label2 setText:[SwichLanguage getString:@"english"]];
...
- (IBAction)clickSureButton:(id)sender {
    ...
    NSString *language = [SwichLanguage userLanguage];
    if ([language isEqualToString:newlanguage]) {
        NSLog(@"相同设置，不需要刷新");
    }else{
        [SwichLanguage setUserlanguage:newlanguage];
...
-(void)didSelectedAtNormalRow:(int)row
{
    ...
    {
        [_delegate didSelectedAtNormalRow:row];
    }
}
